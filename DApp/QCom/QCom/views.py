...
from qcomapp.models import WebUser, Cart, Orders, Transaction
...
def handleSignUpSeller(request):
    if request.method == "POST":
        ...
        web_user = WebUser(
            ...
            group=models.Group.objects.filter(name="Seller")[0])
...
def displayAllTransactions(request):
    all_txns = list(Transaction.objects.all().order_by('-id').values())
    print(all_txns)
    return render(request, 'allTxns.html', {"all_txns": all_txns})

@csrf_exempt
def saveTransaction(request):
    if request.method == "POST":
        request_content = json.loads(request.body.decode('utf-8'))
        txn_hash = request_content.get("txn_hash")
        block_hashes = request_content.get("block_hashes")
        print(txn_hash, block_hashes)
        new_transaction = Transaction(txn_value=txn_hash,block_hashes=block_hashes)
...
def getUnverifiedTransactions(request):
    unverified_txns = list(Transaction.objects.filter(is_verified=False).values())
    return JsonResponse({"all_unverified_txns": unverified_txns})

@csrf_exempt
def updateVerifiedTransactions(request):
    if request.method == "POST":
        request_content = json.loads(request.body.decode('utf-8'))
        verification_transaction_link = request_content.get("txn_link")
        all_verified_txns = request_content.get("verified_txns")

        for txn_hash in all_verified_txns:
            curr_txn = Transaction.objects.filter(txn_value=txn_hash)[0]
            curr_txn.is_verified = True
            curr_txn.l2_verification_link = verification_transaction_link
            curr_txn.save()
        return JsonResponse({"Success":"OK"})
    return HttpResponse("Unauthorized", status=403)

@csrf_exempt
def callQuickFunction(request):
    url = "https://api.quicknode.com/functions/rest/v1/functions/6d6ebb65-a57a-42d3-aa55-85f965b9f994/call?result_only=true"
